...
# ==============================================================================
# --- Model: Lasso Regression (Manual Loop)
# --- Tuning: Manual loop with 10-fold CV and 'r2' scoring
# --- Preprocessing: User-provided script
# ==============================================================================

# Score improved from 19147.346 to 18959.588
# In the next one i will try one final zoom around alpha = 0.0006.

warnings.filterwarnings("ignore")
print("Starting Lasso Regression script...")
...
print("\n" + "=" * 80)
print("Manually tuning Lasso (10-fold CV, R^2 scoring)...")

best_lasso_score = -float('inf')
best_lasso_alpha = 0
# Using a zoomed in list around alpha = 0.0005
alphas_lasso = [0.0004, 0.00041, 0.00042, 0.00043, 0.00044, 0.00045, 0.00046, 
                0.00047, 0.00048, 0.00049, 0.0005, 0.00051, 0.00052, 0.00053, 
                0.00054, 0.00055, 0.00056, 0.00057, 0.00058, 0.00059, 0.0006]

start_time = time.time()

for alpha in alphas_lasso:
    # Using max_iter=10000 as specified
    lasso = Lasso(alpha=alpha, max_iter=20000)
    
    # Using 10-fold CV and 'r2' scoring as specified
...
print("-" * 80)
print(f"Best Lasso Alpha found: {best_lasso_alpha} with R^2 Score: {best_lasso_score:.4f}")
print("-" * 80)


# 3.3 Train Final Model (using the best alpha found in the loop)
print(f"Training final model on all data using best alpha={best_lasso_alpha}...")
...
submission_df.to_csv("submission_lassov2.csv", index=False)

print("\n" + "=" * 80)
print("submission_lassov2.csv has been created!")
